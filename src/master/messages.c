...
static void HandleGetServers(const char *msg, const struct sockaddr_in * addr) {
	...
	// Reset the packet index(no need to change the header)
		...
		if (max_msg_level >= MSG_DEBUG) {
			MsgPrint(MSG_DEBUG, "Comparing server: IP:\"%u.%u.%u.%u:%hu\", p:%u, c:%hu\n", sv_addr >> 24, (sv_addr >> 16) & 0xFF, (sv_addr >> 8) & 0xFF, sv_addr & 0xFF, sv_port, sv->protocol, sv->nbclients);
...
void HandleMessage(const char *msg, size_t length, const struct sockaddr_in * address) {
	...
	if (!strncmp(S2M_HEARTBEAT, msg, strlen(S2M_HEARTBEAT))) {
		...
		server->active = qtrue;
		// If we haven't yet received any infoResponse from this server, // we let it some more time to contact us. After that, only
