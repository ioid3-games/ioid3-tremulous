...
Function to distribute rewards to entities that killed this one. Returns the total damage dealt.
=======================================================================================================================================
*/
...
static float GetRegionDamageModifier(gentity_t *targ, int class, int piece) {
	...
	if (g_debugDamage.integer > 2) {
		G_Printf("GetRegionDamageModifier():\n"
							...
							".   modifier = %f\n", overlap->minAngle, overlap->maxAngle, overlap->minHeight, overlap->maxHeight, overlap->name, overlap->modifier);
	}
	// find the armour layer modifier, assuming that none of the armour regions overlap and that any areas that are not covered have a modifier of 1.0
		...
		for (i = 0; i < g_numArmourRegions[j]; i++) {
			...
			if (g_debugDamage.integer > 2) {
				G_Printf(".   armourRegion = [%d %d %f %f](%s)\n"
									".   .   modifier = %f\n"
									".   .   angleSpan = %f\n"
									".   .   heightSpan = %f\n", regions[i].minAngle, regions[i].maxAngle, regions[i].minHeight, regions[i].maxHeight, regions[i].name, regions[i].modifier, angleSpan, heightSpan);
			}
...
static float GetNonLocDamageModifier(gentity_t *targ, int class) {
	...
	// for every body region, use stretch - armor formula to apply armour modifier for any overlapping area that armour shares with the body region
	crouch = targ->client->ps.pm_flags & PMF_DUCKED;
...
static float GetPointDamageModifier(gentity_t *targ, damageRegion_t *regions, int len, float angle, float height) {
	...
	for (i = 0; i < len; i++) {
		...
		if ((regions[i].minAngle <= regions[i].maxAngle && (angle < regions[i].minAngle || angle > regions[i].maxAngle)) || (regions[i].minAngle > regions[i].maxAngle && angle > regions[i].maxAngle && angle < regions[i].minAngle)) {
			continue;
		}
...
static float G_CalcDamageModifier(vec3_t point, gentity_t *targ, gentity_t *attacker, int class, int dflags) {
	...
	modifier = GetPointDamageModifier(targ, g_damageRegions[class], g_numDamageRegions[class], hitRotation, hitRatio);
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vec3_t dir, vec3_t point, int damage, int dflags, int mod) {
	...
	// too much knockback from falling really far makes you "bounce" and looks silly. However, none at all also looks bad. Cap it.
	...
	if ((mod == MOD_LEVEL4_TRAMPLE || mod == MOD_LEVEL3_POUNCE || mod == MOD_LEVEL4_CRUSH) && targ->s.eType == ET_BUILDABLE && targ->buildableTeam == TEAM_ALIENS) {
		return;
	}
	...
	if (!(dflags & DAMAGE_NO_PROTECTION)) {
		...
		if (targ->s.eType == ET_BUILDABLE && attacker->client && mod != MOD_DECONSTRUCT && mod != MOD_SUICIDE && mod != MOD_REPLACE && mod != MOD_NOCREEP) {
...
void G_LogDestruction(gentity_t *self, gentity_t *actor, int mod) {
	...
	G_LogPrintf(S_COLOR_YELLOW "Deconstruct : %d %d %s %s : %s %s by %s\n", (int)(actor - g_entities), (int)(self - g_entities), BG_Buildable(self->s.modelindex)->name, modNames[mod], BG_Buildable(self->s.modelindex)->humanName, mod == MOD_DECONSTRUCT ? "deconstructed" : "destroyed", actor->client ? actor->client->pers.netname : "<world>");
	// no - power deaths for humans come after some minutes and it's confusing when the messages appear attributed to the deconner. Just don't print them.
