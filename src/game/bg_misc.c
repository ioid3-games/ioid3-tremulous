...
static const buildableAttributes_t bg_buildableList[] = {
	{
		...
		"Egg", // char *humanName;
		"The most basic alien structure. It allows aliens to spawn and protect the Overmind. Without any of these, the Overmind is left nearly defenseless and defeat is imminent.",
	...
	},
	{
		...
		"A collective consciousness that controls all the alien structures in its vicinity. It must be protected at all costs, since its death will render alien structures defenseless.",
	...
	},
	{
		...
		"Used to obstruct corridors and doorways, hindering humans from threatening the spawns and Overmind. Barricades will shrink to allow aliens to pass over them, however.",
	...
	},
	{
		...
		"Fires a blob of adhesive spit at any non - alien in its line of sight. This hinders their movement, making them an easy target for other defensive structures or aliens.",
	...
	},
	{
		...
		"Laces the attacks of any alien that touches it with a poison that will gradually deal damage to any humans exposed to it. The booster also increases the rate of health regeneration for any nearby aliens.",
	...
	},
	{
		...
		"hive", // char *buildName;
		"Hive", // char *humanName;
		"Houses millions of tiny insectoid aliens. When a human approaches this structure, the insectoids attack.",
	...
	},
	{
		...
		"The most basic human structure. It provides a means for humans to enter the battle arena. Without any of these the humans cannot spawn and defeat is imminent.",
	...
	},
	{
		...
		"Automated base defense that is effective against large targets but slow to begin firing. Should always be backed up by physical support.",
	...
	},
	{
		...
		"A structure equipped with a strong electrical attack that fires instantly and always hits its target. It is effective against smaller aliens and for consolidating basic defense.",
	...
	},
	{
		...
		"An essential part of the human base, providing a means to upgrade the basic human equipment. A range of upgrades and weapons are available for sale from the armoury.",
	...
	},
	{
		...
		"A structure that enables self - repair functionality in human structures. Each Defence Computer built increases repair rate slightly.",
	...
	},
	{
		...
		"A structure that automatically restores the health and stamina of any human that stands on it. It may only be used by one person at a time. This structure also issues medkits.",
	...
	},
	{
		...
		"All structures except the telenode rely on a reactor to operate. The reactor provides power for all the human structures either directly or via repeaters. Only one reactor can be built at a time.",
	...
	},
	{
		...
		"A power distributor that transmits power from the reactor to remote locations, so that bases may be built far from the reactor.",
...
static const classAttributes_t bg_classList[] = {
	...
	},
	{
		...
		"Responsible for building and maintaining all the alien structures. Has a weak melee slash attack.", 
		(1 << S1)|(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"Similar to the base Granger, except that in addition to being able to build structures it has a spit attack that slows victims and the ability to crawl on walls.",
		(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"Has a lethal reflexive bite and the ability to crawl on walls and ceilings.", 
		(1 << S1)|(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"A support class able to crawl on walls and ceilings. Its melee attack is most effective when combined with the ability to grab and hold its victims in place. Provides a weak healing aura that accelerates the healing rate of nearby aliens.",
		(1 << S1)|(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"In addition to the basic Basilisk abilities, the Advanced Basilisk sprays a poisonous gas which disorients any nearby humans. Has a strong healing aura that that accelerates the healing rate of nearby aliens.",
		(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"Has a melee attack and the ability to jump off walls. This allows the Marauder to gather great speed in enclosed areas.",
		(1 << S1)|(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"The Advanced Marauder has all the abilities of the basic Marauder with the addition of an area effect electric shock attack.",
		(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"Possesses a melee attack and the pounce ability, which may be used as both an attack and a means to reach remote locations inaccessible from the ground.",
		(1 << S1)|(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"In addition to the basic Dragoon abilities, the Advanced Dragoon has 3 barbs which may be used to attack humans from a distance.",
		(1 << S2)|(1 << S3), // int stages
	...
	},
	{
		...
		"A large alien with a strong melee attack, this class can also charge at enemy humans and structures, inflicting great damage. Any humans or their structures caught under a falling Tyrant will be crushed by its weight.",
		(1 << S3), // int stages
...
const classAttributes_t *BG_Class(class_t class) {
	return (class >= PCL_NONE && class < PCL_NUM_CLASSES) ? &bg_classList[class] : &nullClass;
}
...
=======================================================================================================================================
*/
qboolean BG_ClassAllowedInStage(class_t class, stage_t stage) {
...
=======================================================================================================================================
*/
void BG_ClassBoundingBox(class_t class, vec3_t mins, vec3_t maxs, vec3_t cmaxs, vec3_t dmins, vec3_t dmaxs) {
...
int BG_ClassCanEvolveFromTo(class_t fclass, class_t tclass, int credits, int stage, int cost) {
	...
	for (i = 0; i < bg_numClasses; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			if (thruClass == PCL_NONE || !BG_ClassAllowedInStage(thruClass, stage) || !BG_ClassIsAllowed(thruClass)) {
				continue;
			}
			...
			if (thruClass == tclass) {
				...
				value = BG_ClassCanEvolveFromTo(thruClass, tclass, credits, stage, cost + evolveCost);
			}
...
qboolean BG_AlienCanEvolve(class_t class, int credits, int stage) {
	...
	for (i = 0; i < bg_numClasses; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			if (tclass != PCL_NONE && BG_ClassAllowedInStage(tclass, stage) && BG_ClassIsAllowed(tclass) && credits >= BG_Class(tclass)->cost * ALIEN_CREDITS_PER_KILL) {
				return qtrue;
			}
		}
...
static const weaponAttributes_t bg_weapons[] = {
	...
	},
	{
		...
		"Basic weapon. Cased projectile weapon, with a slow clip based reload system.",
	...
	},
	{
		...
		"Similar to a chainsaw, but instead of a chain it has an electric arc capable of dealing a great deal of damage at close range.",
		0, // int maxAmmo;
		0, // int maxClips;
		qtrue, // int infiniteAmmo;
		qfalse, // int usesEnergy;
	...
	},
	{
		...
		"Close range weapon that is useful against larger foes. It has a slow repeat rate, but can be devastatingly effective.",
	...
	},
	{
		...
		"Slightly more powerful than the basic rifle, rapidly fires small packets of energy.",
	...
	},
	{
		...
		"A portable particle accelerator which causes minor nuclear reactions at the point of impact. It has a very large payload, but fires slowly.",
	...
	},
	{
		...
		"Belt drive, cased projectile weapon. It has a high repeat rate but a wide firing angle and is therefore relatively inaccurate.",
	...
	},
	{
		...
		"Sprays fire at its target. It is powered by compressed gas. The relatively low rate of fire means this weapon is most effective against static targets.",
	...
	},
	{
		...
		"Blaster technology scaled up to deliver devastating power. Primary fire must be charged before firing. It has a quick secondary attack that does not require charging.",
	...
	},
	{
		...
		"Used for building structures. This includes spawns, power and basic defense. More structures become available with new stages.",
		0, // int maxAmmo;
		0, // int maxClips;
		qtrue, // int infiniteAmmo;
		qfalse, // int usesEnergy;
...
static const upgradeAttributes_t bg_upgrades[] = {
	{
		...
		"Protective armour that helps to defend against light alien melee attacks.",
		"icons/iconu_larmour",
		qtrue, // qboolean purchasable
		qfalse, // qboolean usable
		TEAM_HUMANS // team_t team;
	},
	{
		...
		"In addition to protecting your head, the helmet provides a scanner indicating the presence of any friendly or hostile lifeforms and structures in your immediate vicinity.",
		"icons/iconu_helmet",
		qtrue, // qboolean purchasable
		qfalse, // qboolean usable
		TEAM_HUMANS // team_t team;
	...
	},
	{
		...
		"Back - mounted battery pack that permits storage of one and a half times the normal energy capacity for energy weapons.",
	...
	},
	{
		...
		"Back - mounted jet pack that enables the user to fly to remote locations. It is very useful against alien spawns in hard to reach spots.",
	...
	},
	{
		...
		"A full body armour that is highly effective at repelling alien attacks. It allows the user to enter hostile situations with a greater degree of confidence.",
	...
	},
	{
		...
		"Grenade", // char *humanName;
		"A small incendinary device ideal for damaging tightly packed alien structures. Has a five second timer.",
	...
	},
	{
		...
		"Ammunition for the currently held weapon.", 0, qtrue, // qboolean purchasable
		qfalse, // qboolean usable
		TEAM_HUMANS // team_t team;
	}
};
...
=======================================================================================================================================
*/
void BG_PositionBuildableRelativeToPlayer(const playerState_t *ps, const vec3_t mins, const vec3_t maxs, void (*trace)(trace_t *, const vec3_t, const vec3_t, const vec3_t, const vec3_t, int, int), vec3_t outOrigin, vec3_t outAngles, trace_t *tr) {
...
Returns the players current weapon or the weapon they are switching to. Only needs to be used for human weapons.
=======================================================================================================================================
*/
...
void BG_PackEntityNumbers(entityState_t *es, const int *entityNums, int count) {
	...
	if (count > MAX_NUM_PACKED_ENTITY_NUMS) {
		...
		Com_Printf(S_COLOR_YELLOW "WARNING: A maximum of %d entity numbers can be packed, but BG_PackEntityNumbers was passed %d entities", MAX_NUM_PACKED_ENTITY_NUMS, count);
	}
	...
	for (i = 0; i < MAX_NUM_PACKED_ENTITY_NUMS; i++) {
		...
		if (entityNum & ~GENTITYNUM_MASK) {
			Com_Error(ERR_FATAL, "BG_PackEntityNumbers passed an entity number(%d) which exceeds %d bits", entityNum, GENTITYNUM_BITS);
		}
...
int BG_UnpackEntityNumbers(entityState_t *es, int *entityNums, int count) {
	...
	for (i = 0; i < count; i++) {
		...
		switch (i) {
			case 0: *entityNum = es->misc;
				break;
			case 1: *entityNum = es->time;
				break;
			case 2: *entityNum = (es->time >> GENTITYNUM_BITS);
				break;
			case 3: *entityNum = (es->time >> (GENTITYNUM_BITS * 2));
				break;
			case 4: *entityNum = es->time2;
				break;
			case 5: *entityNum = (es->time2 >> GENTITYNUM_BITS);
				break;
			case 6: *entityNum = (es->time2 >> (GENTITYNUM_BITS * 2));
				break;
			case 7: *entityNum = es->constantLight;
				break;
			case 8: *entityNum = (es->constantLight >> GENTITYNUM_BITS);
				break;
			case 9: *entityNum = (es->constantLight >> (GENTITYNUM_BITS * 2));
				break;
			default: Com_Error(ERR_FATAL, "Entity index %d not handled", i);
				break;
		}
