...
static void ClientShove(gentity_t *ent, gentity_t *victim) {
	...
	// set the pmove timer so that the other client can't cancel out the movement immediately
...
void ClientImpacts(gentity_t *ent, pmove_t *pm) {
	...
	for (i = 0; i < pm->numtouch; i++) {
		...
		// if our movement is blocked by another player's real position, don't use the unlagged position for them because they are
...
void SpectatorThink(gentity_t *ent, usercmd_t *ucmd) {
	...
	if (following) {
		...
		if (clientNum < 0 || clientNum > level.maxclients || !g_entities[clientNum].client || g_entities[clientNum].client->sess.spectatorState != SPECTATOR_NOT) {
			following = qfalse;
		}
	}
...
qboolean ClientInactivityTimer(gentity_t *ent) {
	...
	if (!g_inactivity.integer) {
		...
		if (level.time > client->inactivityTime - 10000 && !client->inactivityWarning && !G_admin_permission(ent, ADMF_ACTIVITY)) {
...
void ClientTimerActions(gentity_t *ent, int msec) {
	...
	while (client->time100 >= 100) {
		...
		} else if ((client->ps.stats[STAT_STATE] & SS_SPEEDBOOST) && !(client->buttons & BUTTON_WALKING)) { // walk overrides sprint
		...
		switch (weapon) {
			...
			case WP_HBUILD:
				...
				if ((client->ps.stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT) > BA_NONE) {
					...
					if (G_CanBuild(ent, client->ps.stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT, dist, dummy, dummy2) == IBE_NONE) {
	...
	while (client->time1000 >= 1000) {
		...
		if (client->ps.stats[STAT_TEAM] == TEAM_ALIENS && level.surrenderTeam == TEAM_ALIENS) {
			G_Damage(ent, NULL, NULL, NULL, NULL, BG_Class(client->ps.stats[STAT_CLASS])->regenRate, DAMAGE_NO_ARMOR, MOD_SUICIDE);
	...
	for (i = oldEventSequence; i < client->ps.eventSequence; i++) {
		...
		switch (event) {
			...
			case EV_FALL_FAR:
				...
				damage = (int)((float)BG_Class(class)->health * BG_Class(class)->fallDamage * fallDistance);
...
Called on every server frame. Stores position data for the client at that into client->unlaggedHist[] and the time into
level.unlaggedTimes[]. This data is used by G_UnlaggedCalc().
=======================================================================================================================================
*/
...
Mark all unlaggedHist[] markers for this client invalid.  Useful for preventing teleporting and death.
=======================================================================================================================================
*/
...
Loops through all active clients and calculates their predicted position for time then stores it in client->unlaggedCalc.
=======================================================================================================================================
*/
void G_UnlaggedCalc(int time, gentity_t *rewindEnt) {
	...
	if (i == MAX_UNLAGGED_MARKERS) {
		// if we searched all markers and the oldest one still isn't old enough just use the oldest marker with no lerping
		lerp = 0.0f;
	}
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		VectorLerp2(lerp, ent->client->unlaggedHist[startIndex].mins, ent->client->unlaggedHist[stopIndex].mins, ent->client->unlaggedCalc.mins);
		VectorLerp2(lerp, ent->client->unlaggedHist[startIndex].maxs, ent->client->unlaggedHist[stopIndex].maxs, ent->client->unlaggedCalc.maxs);
		VectorLerp2(lerp, ent->client->unlaggedHist[startIndex].origin, ent->client->unlaggedHist[stopIndex].origin, ent->client->unlaggedCalc.origin);
...
cgame prediction will predict a client's own position all the way up to the current time, but only updates other player's positions up
to the postition sent in the most recent snapshot.

This allows player X to essentially "move through" the position of player Y when player X's cmd is processed with Pmove() on the server.
This is because player Y was clipping player X's Pmove() on his client, but when the same cmd is processed with Pmove on the server it
is not clipped.

Long story short(too late) : don't use unlagged positions for players who were blocking this player X's client - side Pmove().
This makes the assumption that if player X's movement was blocked in the client he's going to still be up against player Y when the
Pmove() is run on the server with the same cmd.
...
//		G_Printf("serverTime >>>>>\n");
	...
	if ((client->ps.eFlags & EF_POISONCLOUDED) && BG_PlayerPoisonCloudTime(&client->ps) - level.time + client->lastPoisonCloudedTime <= 0)
	...
	if (level.surrenderTeam != client->pers.teamSelection && ent->nextRegenTime >= 0 && ent->nextRegenTime < level.time) {
		...
		if (ent->health <= 0 || ent->nextRegenTime < 0 || regenRate == 0) {
			...
			for (i = 0; i < num; i++) {
				...
				if (modifier < BOOSTER_REGEN_MOD && boost->s.eType == ET_BUILDABLE && boost->s.modelindex == BA_A_BOOSTER && boost->spawned && boost->health > 0 && boost->powered) {
				...
				if (boost->s.eType == ET_PLAYER && boost->client && boost->client->pers.teamSelection == ent->client->pers.teamSelection && boost->health > 0) {
					...
					if (class == PCL_ALIEN_LEVEL1 && modifier < LEVEL1_REGEN_MOD) {
						...
						didBoost = qtrue;
					} else if (class == PCL_ALIEN_LEVEL1_UPG && modifier < LEVEL1_UPG_REGEN_MOD) {
