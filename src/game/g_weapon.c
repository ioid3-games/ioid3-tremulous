...
void LCChargeFire(gentity_t *ent, qboolean secondary) {
	...
	if (secondary && ent->client->ps.stats[STAT_MISC] <= 0) {
		fire_luciferCannon(ent, muzzle, forward, LCANNON_SECONDARY_DAMAGE, LCANNON_SECONDARY_RADIUS, LCANNON_SECONDARY_SPEED);
	} else {
		fire_luciferCannon(ent, muzzle, forward, ent->client->ps.stats[STAT_MISC] * LCANNON_DAMAGE / LCANNON_CHARGE_TIME_MAX, LCANNON_RADIUS, LCANNON_SPEED);
	}
...
void CheckCkitRepair(gentity_t *ent) {
	...
	if (tr.fraction < 1.0f && traceEnt->spawned && traceEnt->health > 0 && traceEnt->s.eType == ET_BUILDABLE && traceEnt->buildableTeam == TEAM_HUMANS) {
...
static void G_FindZapChainTargets(zap_t *zap) {
	...
	for (i = 0; i < num; i++) {
		...
		if (((enemy->client && enemy->client->ps.stats[STAT_TEAM] == TEAM_HUMANS) || (enemy->s.eType == ET_BUILDABLE && BG_Buildable(enemy->s.modelindex)->team == TEAM_HUMANS)) && enemy->health > 0 && distance <= LEVEL2_AREAZAP_CHAIN_RANGE) { // only chain to living targets
			// world - LOS check : trace against the world, ignoring other BODY entities
			trap_Trace(&tr, ent->s.origin, NULL, NULL, enemy->s.origin, ent->s.number, CONTENTS_SOLID);
...
static void G_CreateNewZap(gentity_t *creator, gentity_t *target) {
	...
	for (i = 0; i < MAX_ZAPS; i++) {
		...
		if (target->health > 0) {
			G_Damage(target, creator, creator, forward, target->s.origin, LEVEL2_AREAZAP_DMG, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_LOCDAMAGE, MOD_LEVEL2_ZAP);
			G_FindZapChainTargets(zap);

			for (i = 1; i < zap->numTargets; i++) {
				G_Damage(zap->targets[i], target, zap->creator, forward, target->s.origin, LEVEL2_AREAZAP_DMG * (1 - pow((zap->distances[i] / LEVEL2_AREAZAP_CHAIN_RANGE), LEVEL2_AREAZAP_CHAIN_FALLOFF)) + 1, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_LOCDAMAGE, MOD_LEVEL2_ZAP);
			}
		}
...
void areaZapFire(gentity_t *ent) {
	...
	if ((traceEnt->client && traceEnt->client->ps.stats[STAT_TEAM] == TEAM_HUMANS) || (traceEnt->s.eType == ET_BUILDABLE && BG_Buildable(traceEnt->s.modelindex)->team == TEAM_HUMANS)) {
...
void G_ChargeAttack(gentity_t *ent, gentity_t *victim) {
	...
	// for buildables, track the last MAX_TRAMPLE_BUILDABLES_TRACKED buildables hit, and do not do damage if the current buildable is in
	// that list in order to prevent dancing over stuff to kill it very quickly
	if (!victim->client) {
		...
		ent->client->trampleBuildablesHit[ent->client->trampleBuildablesHitPos++ % MAX_TRAMPLE_BUILDABLES_TRACKED] = victim - g_entities;
	}
	...
	G_Damage(victim, ent, ent, forward, victim->s.origin, damage, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL4_TRAMPLE);
...
void G_CrushAttack(gentity_t *ent, gentity_t *victim) {
	...
	if (!victim->takedamage || ent->client->ps.origin[2] + ent->r.mins[2] < victim->s.origin[2] + victim->r.maxs[2] || (victim->client && victim->client->ps.groundEntityNum == ENTITYNUM_NONE)) {
		return;
	}
	...
	G_Damage(victim, ent, ent, dir, victim->s.origin, damage, DAMAGE_NO_LOCDAMAGE, MOD_LEVEL4_CRUSH);
}
...
void FireWeapon(gentity_t *ent) {
	...
	switch (ent->s.weapon) {
		case WP_ALEVEL1:
			...
			break;
		case WP_ALEVEL1_UPG:
			meleeAttack(ent, LEVEL1_CLAW_U_RANGE, LEVEL1_CLAW_WIDTH, LEVEL1_CLAW_WIDTH, LEVEL1_CLAW_DMG, MOD_LEVEL1_CLAW);
			break;
		case WP_ALEVEL3:
		...
		case WP_ALEVEL2:
			...
			break;
		case WP_ALEVEL2_UPG:
			meleeAttack(ent, LEVEL2_CLAW_U_RANGE, LEVEL2_CLAW_WIDTH, LEVEL2_CLAW_WIDTH, LEVEL2_CLAW_DMG, MOD_LEVEL2_CLAW);
			break;
		case WP_ALEVEL4:
