...
static void G_FindZapChainTargets(zap_t *zap) {
	...
	for (i = 0; i < num; i++) {
		...
		if (((enemy->client && enemy->client->ps.stats[STAT_TEAM] == TEAM_HUMANS) || (enemy->s.eType == ET_BUILDABLE && BG_Buildable(enemy->s.modelindex)->team == TEAM_HUMANS)) && enemy->health > 0 && // only chain to living targets
...
void G_ChargeAttack(gentity_t *ent, gentity_t *victim) {
	...
	if (!victim->client) {
		...
		ent->client->trampleBuildablesHit[
			ent->client->trampleBuildablesHitPos++ % MAX_TRAMPLE_BUILDABLES_TRACKED] = victim - g_entities;
	}
