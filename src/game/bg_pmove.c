...
static float PM_CmdScale(usercmd_t *cmd) {
	...
	if (BG_ClassHasAbility(pm->ps->stats[STAT_CLASS], SCA_STAMINA) && pm->ps->pm_type == PM_NORMAL) {
		...
		// walk overrides sprint. We keep the state that we want to be sprinting (above), but don't apply the modifier, and in g_active
		// we skip taking the stamina too.
...
static qboolean PM_CheckJump(void) {
	...
	if (BG_ClassHasAbility(pm->ps->stats[STAT_CLASS], SCA_STAMINA) && (pm->ps->stats[STAT_STAMINA] < STAMINA_SLOW_LEVEL + STAMINA_JUMP_TAKE)) {
		return qfalse;
	}
	// no bunny hopping off a dodge
	// SCA_DODGE? -vjr
	if (BG_ClassHasAbility(pm->ps->stats[STAT_CLASS], SCA_STAMINA) && pm->ps->pm_time) {
		return qfalse;
	}
...
static qboolean PM_CheckDodge(void) {
	...
	if (pm->ps->pm_type != PM_NORMAL || pm->ps->stats[STAT_STAMINA] < STAMINA_SLOW_LEVEL + STAMINA_DODGE_TAKE || (pm->ps->pm_flags & PMF_DUCKED)) {
		return qfalse;
	}
	// can't dodge forward
	if (pm->cmd.forwardmove > 0) {
	...
	if ((pm->ps->stats[STAT_STATE] & SS_SLOWLOCKED) || (pm->ps->stats[STAT_STATE] & SS_CREEPSLOWED) || (pm->ps->eFlags & EF_POISONCLOUDED)) {
	...
	for (i = 0; i < 3; i++) {
		if ((velocity[i] < 0.0f && pm->ps->velocity[i] > velocity[i]) || (velocity[i] > 0.0f && pm->ps->velocity[i] < velocity[i])) {
	...
	for (i = 0; i <= 4; i++) {
		switch (i) {
			...
			case 1:
				...
				// mask out CONTENTS_BODY to not hit other players and avoid the camera flipping out when wallwalkers touch
		...
		if (trace.fraction < 1.0f && !(trace.surfaceFlags &(SURF_SKY|SURF_SLICK)) && !(trace.entityNum != ENTITYNUM_WORLD && i != 4)) {
			...
			if (!VectorCompareEpsilon(trace.plane.normal, surfNormal, eps)) {
				...
				if (!VectorCompareEpsilon(trace.plane.normal, refNormal, eps) && !VectorCompareEpsilon(surfNormal, refNormal, eps) && !VectorCompareEpsilon(trace.plane.normal, ceilingNormal, eps) && !VectorCompareEpsilon(surfNormal, ceilingNormal, eps)) {
...
static void PM_SetViewheight(void) {
	pm->ps->viewheight = (pm->ps->pm_flags & PMF_DUCKED) ? BG_ClassConfig(pm->ps->stats[STAT_CLASS])->crouchViewheight : BG_ClassConfig(pm->ps->stats[STAT_CLASS])->viewheight;
}
...
static void PM_CheckDuck(void) {
	...
	if ((pm->cmd.upmove < 0) && !VectorCompare(PCmaxs, PCcmaxs) && pm->ps->pm_type != PM_JETPACK) { // duck
...
static void PM_Weapon(void) {
	...
	if (!BG_Weapon(pm->ps->weapon)->infiniteAmmo && (pm->ps->ammo <= 0 || (pm->ps->pm_flags & PMF_WEAPON_RELOAD)) && pm->ps->clips > 0) {
		pm->ps->pm_flags &= ~PMF_WEAPON_RELOAD;
	...
	switch (pm->ps->weapon) {
		...
		case WP_ALEVEL3:
		case WP_ALEVEL3_UPG:
			// pouncing has primary secondary AND autohit procedures pounce is autohit
	...
	if (!(pm->ps->persistant[PERS_STATE] & PS_NONSEGMODEL)) {
		...
		int num = rand();
		// FIXME: it would be nice to have these hard coded policies in weapon.cfg
		switch (pm->ps->weapon) {
			case WP_ALEVEL1_UPG:
			case WP_ALEVEL1:
				if (attack1) {
...
void PM_UpdateViewAngles(playerState_t *ps, const usercmd_t *cmd) {
	...
	if (!(ps->stats[STAT_STATE] & SS_WALLCLIMBING) || !BG_RotateAxis(ps->grapplePoint, axis, rotaxis, qfalse, ps->eFlags & EF_WALLCLIMBCEILING)) {
...
void PmoveSingle(pmove_t *pmove) {
	...
	if (!(pm->ps->pm_flags & PMF_RESPAWNED) && pm->ps->pm_type != PM_INTERMISSION && (pm->cmd.buttons & BUTTON_ATTACK) && ((pm->ps->ammo > 0 || pm->ps->clips > 0) || BG_Weapon(pm->ps->weapon)->infiniteAmmo)) {
		pm->ps->eFlags |= EF_FIRING;
	} else {
		pm->ps->eFlags &= ~EF_FIRING;
	}
	// set the firing flag for continuous beam weapons
	if (!(pm->ps->pm_flags & PMF_RESPAWNED) && pm->ps->pm_type != PM_INTERMISSION && (pm->cmd.buttons & BUTTON_ATTACK2) && ((pm->ps->ammo > 0 || pm->ps->clips > 0) || BG_Weapon(pm->ps->weapon)->infiniteAmmo)) {
		pm->ps->eFlags |= EF_FIRING2;
	} else {
		pm->ps->eFlags &= ~EF_FIRING2;
	}
	// set the firing flag for continuous beam weapons
	if (!(pm->ps->pm_flags & PMF_RESPAWNED) && pm->ps->pm_type != PM_INTERMISSION && (pm->cmd.buttons & BUTTON_USE_HOLDABLE) && ((pm->ps->ammo > 0 || pm->ps->clips > 0) || BG_Weapon(pm->ps->weapon)->infiniteAmmo)) {
	...
	if (pm->ps->pm_type == PM_JETPACK) {
		...
		if (BG_ClassHasAbility(pm->ps->stats[STAT_CLASS], SCA_WALLCLIMBER) && (pm->ps->stats[STAT_STATE] & SS_WALLCLIMBING)) {
