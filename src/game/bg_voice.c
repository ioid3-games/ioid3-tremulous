...
static voice_t *BG_VoiceList(void) {
	...
	for (i = 0; i < numFiles; i++, filePtr += fileLen + 1) {
		...
		if (count >= MAX_VOICES) {
			Com_Printf(S_COLOR_YELLOW "WARNING: .voice file overflow. %d of %d .voice files loaded. MAX_VOICES is %d", count, numFiles, MAX_VOICES);
			break;
		}
	...
	while (foundToken) {
		if (token.string[0] == '}') {
			if (foundText) {
				...
				BG_VoiceParseError(handle, "BG_VoiceParseTrack(): missing text attribute for track");
			}
			...
			if (foundText) {
				BG_VoiceParseError(handle, "BG_VoiceParseTrack(): duplicate \"text\" definition for track");
			}

			foundToken = trap_Parse_ReadToken(handle, &token);

			if (!foundToken) {
				BG_VoiceParseError(handle, "BG_VoiceParseTrack(): missing \"text\" value");
			}

			foundText = qtrue;

			if (strlen(token.string) >= MAX_SAY_TEXT) {
				BG_VoiceParseError(handle, va("BG_VoiceParseTrack(): \"text\" value" "\"%s\"exceeds MAX_SAY_TEXT length", token.string));
			}
			...
			if (token.type == TT_NUMBER) {
				...
				BG_VoiceParseError(handle, "BG_VoiceParseTrack(): "missing \"enthusiasm\" value");
			}

			foundToken = trap_Parse_ReadToken(handle, &token);
			continue;
		} else {
			BG_VoiceParseError(handle, va("BG_VoiceParseTrack(): unknown token \"%s\"", token.string));
		}
	}
	...
	while (trap_Parse_ReadToken(handle, &token)) {
		...
		if (parsingTrack) {
			if (token.string[0] == '{') {
				...
				BG_VoiceParseError(handle, va("BG_VoiceParseCommand(): parse error at \"%s\"", token.string));
			}
		}
		...
		if (!trap_FS_FOpenFile(token.string, NULL, FS_READ)) {
			...
			trap_Parse_SourceFileAndLine(handle, filename, &line);
			Com_Printf(S_COLOR_YELLOW "WARNING: BG_VoiceParseCommand(): track \"%s\" referenced on line %d of %s does not exist\n", token.string, line, filename);
		} else {
#ifdef CGAME
	...
	while (trap_Parse_ReadToken(handle, &token)) {
		if (parsingCmd) {
			if (token.string[0] == '{') {
				...
				trap_Parse_SourceFileAndLine(handle, filename, &line);
				Com_Error(ERR_FATAL, "BG_VoiceParse(): parse error on line %d of %s\n", line, filename);
			}
		}
		...
		if (strlen(token.string) >= MAX_VOICE_CMD_LEN) {
			...
			trap_Parse_SourceFileAndLine(handle, filename, &line);
			Com_Error(ERR_FATAL, "BG_VoiceParse(): command \"%s\" exceeds MAX_VOICE_CMD_LEN(%d) on line %d of %s\n", token.string, MAX_VOICE_CMD_LEN, line, filename);
		}
...
=======================================================================================================================================
*/
voiceTrack_t *BG_VoiceTrackFind(voiceTrack_t *head, team_t team, class_t class, weapon_t weapon, int enthusiasm, int *trackNum) {
	voiceTrack_t *vt = head;
	int highestMatch = 0;
	int matchCount = 0;
	int selectedMatch = 0;
	...
	while (vt) {
		if ((vt->team >= 0 && !(vt->team &(1 << team))) || (vt->class >= 0 && !(vt->class &(1 << class))) || (vt->weapon >= 0 && !(vt->weapon &(1 << weapon))) || vt->enthusiasm > enthusiasm) {
			vt = vt->next;
			continue;
		}
	...
	while (vt) {
		...
		if ((vt->team >= 0 && !(vt->team &(1 << team))) || (vt->class >= 0 && !(vt->class &(1 << class))) || (vt->weapon >= 0 && !(vt->weapon &(1 << weapon))) || vt->enthusiasm != highestMatch) {
			vt = vt->next;
			continue;
		}
