...
(int)(ent ? ent - g_entities : - 1), mode, message));
...
void Cmd_VSay_f(gentity_t *ent) {
  ...
  cmd = BG_VoiceCmdFind(voice->cmds, voiceCmd, &cmdNum);
	...
	if (!track) {
		trap_SendServerCommand(ent - g_entities, va("print \"%s : no available track for command '%s', team %d, "
			"class %d, weapon %d, and enthusiasm %d in voice '%s'\n\"", vsay, voiceCmd, ent->client->pers.teamSelection, ent->client->pers.classSelection, weapon, 
		...
		if (!Q_stricmp(vote, "kick") || !Q_stricmp(vote, "mute") ||
			...
			if (G_admin_permission(g_entities + clientNum, ADMF_IMMUNITY)) {
				...
				G_AdminMessage(NULL, va(S_COLOR_WHITE "%s" S_COLOR_YELLOW " attempted %s %s"
																...
																" for : %s", ent->client->pers.netname, cmd, vote, g_entities[clientNum].client->pers.netname, reason[0] ? reason : "no reason"));
				return;
			}
...
(ent->client->pers.teamSelection != 
	...
	if (!Q_stricmp(vote, "kick")) {
		...
		trap_SendServerCommand(ent - g_entities, "print \"Invalid vote string\n\"");
		trap_SendServerCommand(ent - g_entities, "print \"Valid team vote commands are : "
...
(level.clients[i].pers.teamSelection == TEAM_NONE && G_admin_permission(&g_entities[i], ADMF_SPEC_ALLCHAT))) {
					...
					trap_SendServerCommand(i, va("chat - 1 %d \"" S_COLOR_YELLOW "%s" 
						S_COLOR_YELLOW " called a team vote(%ss) : %s\"", SAY_ADMINS, ent->client->pers.netname, BG_TeamName(team), level.voteDisplayString[team]));
				}
			}
		}
	}
...
((1 << ent->client->ps.weapon) & BG_Buildable(buildable)->buildWeapon) && BG_BuildableIsAllowed(buildable) && 
...
(int)((ent) ? ent - g_entities : - 1), 
(ent) ? ent->client->pers.netname : "console", name, color, msg);
	}
}
