...
Returns - 1 and optionally sets err if invalid or not exactly 1 match. err will have a trailing \n if set.
=======================================================================================================================================
*/
...
Sets plist to an array of integers that represent client numbers that have names that are a partial match for s.
...
void ScoreboardMessage(gentity_t *ent) {
	...
	for (i = 0; i < numSorted; i++) {
		...
		if (cl->sess.spectatorState == SPECTATOR_NOT && (ent->client->pers.teamSelection == TEAM_NONE || cl->pers.teamSelection == ent->client->pers.teamSelection)) {
			weapon = cl->ps.weapon;
...
Duplicate of concatargs but enquotes things that need to be. Used to log command arguments in a way that preserves user intended
tokenizing.
=======================================================================================================================================
*/
...
static void Give_Class(gentity_t *ent, char *s) {
	...
	ent->client->pers.evolveHealthFraction = (float)ent->client->ps.stats[STAT_HEALTH] / (float)BG_Class(currentClass)->health;
...
void Cmd_Give_f(gentity_t *ent) {
	...
	// change class - this allows you to be any alien class on TEAM_HUMAN and the otherway round.
	Give_Class(ent, name);
	...
	if (give_all || Q_stricmp(name, "ammo") == 0) {
		...
		if (client->ps.weapon != WP_ALEVEL3_UPG && BG_Weapon(client->ps.weapon)->infiniteAmmo) {
			return;
		}
		...
		if (BG_Weapon(client->ps.weapon)->usesEnergy && BG_InventoryContainsUpgrade(UP_BATTPACK, client->ps.stats)) {
...
void Cmd_Team_f(gentity_t *ent) {
	...
	// stop switching teams for gameplay exploit reasons by enforcing a long wait before they can come back
	if (!force && !g_cheats.integer && ent->client->pers.aliveSeconds && level.time - ent->client->pers.teamChangeTime < 30000) {
		trap_SendServerCommand(ent - g_entities, va("print \"You must wait another %d seconds before changing teams again\n\"", (int)((30000 - (level.time - ent->client->pers.teamChangeTime)) / 1000.f)));
		return;
	}
...
static qboolean G_SayTo(gentity_t *ent, gentity_t *other, saymode_t mode, const char *message) {
	...
	trap_SendServerCommand(other - g_entities, va("chat %d %d \"%s\"", (int)(ent ? ent - g_entities : - 1), mode, message));
...
void G_Say(gentity_t *ent, saymode_t mode, const char *chatText) {
	...
	if ((!g_specChat.integer) && (mode != SAY_TEAM) && (ent) && (ent->client->pers.teamSelection == TEAM_NONE) && (!G_admin_permission(ent, ADMF_NOCENSORFLOOD))) {
		trap_SendServerCommand(ent - g_entities, "print \"say : Global chatting for spectators has been disabled. You may only use team chat.\n\"");
		mode = SAY_TEAM;
	}

	switch (mode) {
		case SAY_ALL:
			G_LogPrintf("Say : %d \"%s" S_COLOR_WHITE "\" : " S_COLOR_GREEN "%s\n", (int)((ent) ? ent - g_entities : - 1), (ent) ? ent->client->pers.netname : "console", chatText);
			break;
		case SAY_TEAM:
...
static void Cmd_SayArea_f(gentity_t *ent) {
	...
	for (i = 0; i < level.maxclients; i++) {
		if (g_entities[i].client->pers.teamSelection == TEAM_NONE && G_admin_permission(&g_entities[i], ADMF_SPEC_ALLCHAT)) {
...
void Cmd_VSay_f(gentity_t *ent) {
	...
	track = BG_VoiceTrackFind(cmd->tracks, ent->client->pers.teamSelection, ent->client->pers.classSelection, weapon, (int)ent->client->voiceEnthusiasm, &trackNum);

	if (!track) {
		trap_SendServerCommand(ent - g_entities, va("print \"%s : no available track for command '%s', team %d, class %d, weapon %d, and enthusiasm %d in voice '%s'\n\"", vsay, voiceCmd, ent->client->pers.teamSelection, ent->client->pers.classSelection, weapon, (int)ent->client->voiceEnthusiasm, voiceName));
		return;
	}
	...
	switch (vchan) {
		...
		case VOICE_CHAN_LOCAL:
			trap_SendServerCommand(-1, va("voice %d %d %d %d \"%s\"\n", (int)(ent - g_entities), vchan, cmdNum, trackNum, text));
			break;
		case VOICE_CHAN_TEAM:
			G_TeamCommand(ent->client->pers.teamSelection, va("voice %d %d %d %d \"%s\"\n", (int)(ent - g_entities), vchan, cmdNum, trackNum, text));
			break;
		default:
			break;
	}
...
void Cmd_CallVote_f(gentity_t *ent) {
	...
	if (g_voteLimit.integer > 0 && ent->client->pers.namelog->voteCount >= g_voteLimit.integer && !G_admin_permission(ent, ADMF_NO_VOTE_LIMIT)) {
	...
	if (!Q_stricmp(vote, "kick") || !Q_stricmp(vote, "mute") || !Q_stricmp(vote, "unmute") || !Q_stricmp(vote, "denybuild") || !Q_stricmp(vote, "allowbuild")) {
		...
		if (!Q_stricmp(vote, "kick") || !Q_stricmp(vote, "mute") || !Q_stricmp(vote, "denybuild")) {
			if (G_admin_permission(g_entities + clientNum, ADMF_IMMUNITY)) {
				...
				G_AdminMessage(NULL, va(S_COLOR_WHITE "%s" S_COLOR_YELLOW " attempted %s %s" " on immune admin " S_COLOR_WHITE "%s" S_COLOR_YELLOW " for : %s", ent->client->pers.netname, cmd, vote, g_entities[clientNum].client->pers.netname, reason[0] ? reason : "no reason"));
				return;
			}
	...
	if (!Q_stricmp(vote, "kick")) {
		...
		Com_sprintf(level.voteString[team], sizeof(level.voteString[team]), "ban %s \"1s%s\" vote kick(%s)", level.clients[clientNum].pers.ip.str, g_adminTempBan.string, reason);
		...
		if (!Q_stricmp(vote, "mute")) {
			...
			Com_sprintf(level.voteDisplayString[team], sizeof(level.voteDisplayString[team]), "Unmute player '%s'", name);
			...
			if (!G_MapExists(arg)) {
				trap_SendServerCommand(ent - g_entities, va("print \"%s : 'maps/%s.bsp' could not be found on the server\n\"", cmd, arg));
				return;
			}

			Com_sprintf(level.voteString[team], sizeof(level.voteString), "%s \"%s\"", vote, arg);
			Com_sprintf(level.voteDisplayString[team], sizeof(level.voteDisplayString[team]), "Change to map '%s'", arg);
			level.voteDelay[team] = 3000;
		} else if (!Q_stricmp(vote, "nextmap")) {
			if (G_MapExists(g_nextMap.string)) {
				trap_SendServerCommand(ent - g_entities, va("print \"%s : the next map is already set to '%s'\n\"", cmd, g_nextMap.string));
				return;
			}
			...
			Com_sprintf(level.voteString[team], sizeof(level.voteString[team]), "set g_nextMap \"%s\"", arg);
			...
			Com_sprintf(level.voteString[team], sizeof(level.voteString[team]), "suddendeath %d", g_suddenDeathVoteDelay.integer);
			Com_sprintf(level.voteDisplayString[team], sizeof(level.voteDisplayString[team]), "Begin sudden death in %d seconds", g_suddenDeathVoteDelay.integer);
		} else {
		...
		Com_sprintf(level.voteDisplayString[team], sizeof(level.voteDisplayString[team]), "Take away building rights from '%s'", name);
	...
	G_LogPrintf("%s : %d \"%s" S_COLOR_WHITE "\" : %s\n", team == TEAM_NONE ? "CallVote" : "CallTeamVote", (int)(ent - g_entities), ent->client->pers.netname, level.voteString[team]);
	...
	if (team == TEAM_NONE) {
		...
		for (i = 0; i < level.maxclients; i++) {
			if (level.clients[i].pers.connected == CON_CONNECTED) {
				if (level.clients[i].pers.teamSelection == team || (level.clients[i].pers.teamSelection == TEAM_NONE && G_admin_permission(&g_entities[i], ADMF_SPEC_ALLCHAT))) {
					...
					trap_SendServerCommand(i, va("chat - 1 %d \"" S_COLOR_YELLOW "%s" S_COLOR_YELLOW " called a team vote(%ss) : %s\"", SAY_ADMINS, ent->client->pers.netname, BG_TeamName(team), level.voteDisplayString[team]));
				}
			}
		}
	}
	...
	if (ent->client->pers.teamSelection == TEAM_ALIENS) {
		...
		if (ent->client->pers.classSelection != PCL_NONE) {
			...
			for (i = 0; i < num; i++) {
				...
				if ((other->client && other->client->ps.stats[STAT_TEAM] == TEAM_HUMANS) || (other->s.eType == ET_BUILDABLE && other->buildableTeam == TEAM_HUMANS && other->powered)) {
			...
			if (ent->client->sess.spectatorState == SPECTATOR_NOT && (currentClass == PCL_ALIEN_BUILDER0 || currentClass == PCL_ALIEN_BUILDER0_UPG) && ent->client->ps.stats[STAT_MISC] > 0) {
...
void Cmd_Destroy_f(gentity_t *ent) {
	...
	if (tr.fraction < 1.0f && (traceEnt->s.eType == ET_BUILDABLE) && (traceEnt->buildableTeam == ent->client->pers.teamSelection) && ((ent->client->ps.weapon >= WP_ABUILD) && (ent->client->ps.weapon <= WP_HBUILD))) {
		...
		if (!g_markDeconstruct.integer || (ent->client->pers.teamSelection == TEAM_HUMANS && !G_FindPower(traceEnt, qtrue))) {
		...
		if (traceEnt->health > 0) {
			if (!deconstruct) {
			...
			} else if (g_markDeconstruct.integer && (ent->client->pers.teamSelection != TEAM_HUMANS || G_FindPower(traceEnt, qtrue) || lastSpawn)) {
				...
				if (!g_cheats.integer) { // add a bit to the build timer
					ent->client->ps.stats[STAT_MISC] += BG_Buildable(traceEnt->s.modelindex)->buildTime / 4;
				}
...
void Cmd_ToggleItem_f(gentity_t *ent) {
	...
	if (weapon != WP_NONE) {
		...
		// special case to allow switching between the blaster and the primary weapon
...
void Cmd_Buy_f(gentity_t *ent) {
	...
	if (G_BuildableRange(ent->client->ps.origin, 100, BA_H_ARMOURY)) {
		energyOnly = qfalse;
	} else if (upgrade == UP_AMMO && BG_Weapon(ent->client->ps.stats[STAT_WEAPON])->usesEnergy && (G_BuildableRange(ent->client->ps.origin, 100, BA_H_REACTOR) || G_BuildableRange(ent->client->ps.origin, 100, BA_H_REPEATER))) {
		energyOnly = qtrue;
	} else {
...
void Cmd_Build_f(gentity_t *ent) {
	...
	if (buildable != BA_NONE && ((1 << ent->client->ps.weapon) & BG_Buildable(buildable)->buildWeapon) && BG_BuildableIsAllowed(buildable) && ((team == TEAM_ALIENS && BG_BuildableAllowedInStage(buildable, g_alienStage.integer)) || (team == TEAM_HUMANS && BG_BuildableAllowedInStage(buildable, g_humanStage.integer)))) {
		dynMenu_t err;
...
Stops any other clients from following this one called when a player leaves a team or dies.
=======================================================================================================================================
*/
void G_StopFromFollowing(gentity_t *ent) {
	...
	for (i = 0; i < level.maxclients; i++) {
		if (level.clients[i].sess.spectatorState == SPECTATOR_FOLLOW && level.clients[i].sess.spectatorClient == ent->client->ps.clientNum) {
...
void G_StopFollowing(gentity_t *ent) {
	...
	if (ent->client->pers.teamSelection == TEAM_NONE) {
		ent->client->sess.spectatorState = ent->client->ps.persistant[PERS_SPECSTATE] = SPECTATOR_FREE;
	} else {
		vec3_t spawn_origin, spawn_angles;

		ent->client->sess.spectatorState = ent->client->ps.persistant[PERS_SPECSTATE] = SPECTATOR_LOCKED;
...
Client is still following a player, but that player has gone to spectator mode and cannot be followed for the moment.
=======================================================================================================================================
*/
void G_FollowLockView(gentity_t *ent) {
	...
	ent->client->sess.spectatorState = ent->client->ps.persistant[PERS_SPECSTATE] = SPECTATOR_FOLLOW;
...
qboolean G_FollowNewClient(gentity_t *ent, int dir) {
	...
	do {
		...
		if (ent->client->pers.teamSelection != TEAM_NONE && (level.clients[clientnum].pers.teamSelection != ent->client->pers.teamSelection)) {
			continue;
		}
...
void Cmd_Follow_f(gentity_t *ent) {
	...
	if (trap_Argc() != 2) {
		...
		if (ent->client->pers.teamSelection != TEAM_NONE && (level.clients[i].pers.teamSelection != ent->client->pers.teamSelection)) {
			return;
		}
...
static void Cmd_Ignore_f(gentity_t *ent) {
	...
	if (trap_Argc() < 2) {
		trap_SendServerCommand(ent - g_entities, va("print \"[skipnotify] usage : %s [clientNum|partial name match]\n\"", cmd));
		return;
	}
	...
	if (matches < 1) {
		trap_SendServerCommand(ent - g_entities, va("print \"[skipnotify] %s : no clients match the name '%s'\n\"", cmd, name));
		return;
	}
	...
	for (i = 0; i < matches; i++) {
		if (ignore) {
			if (!Com_ClientListContains(&ent->client->sess.ignoreList, pids[i])) {
				...
				ClientUserinfoChanged(ent->client->ps.clientNum, qfalse);
				trap_SendServerCommand(ent - g_entities, va("print \"[skipnotify] ignore : added %s^7 to your ignore list\n\"", level.clients[pids[i]].pers.netname));
			} else {
				trap_SendServerCommand(ent - g_entities, va("print \"[skipnotify] ignore : %s^7 is already on your ignore list\n\"", level.clients[pids[i]].pers.netname));
			}
		} else {
			if (Com_ClientListContains(&ent->client->sess.ignoreList, pids[i])) {
				Com_ClientListRemove(&ent->client->sess.ignoreList, pids[i]);
				ClientUserinfoChanged(ent->client->ps.clientNum, qfalse);
				trap_SendServerCommand(ent - g_entities, va("print \"[skipnotify] unignore : removed %s^7 from your ignore list\n\"", level.clients[pids[i]].pers.netname));
			} else {
				trap_SendServerCommand(ent - g_entities, va("print \"[skipnotify] unignore : %s^7 is not on your ignore list\n\"", level.clients[pids[i]].pers.netname));
			}
		}
	}
}
...
Determine whether a user is flood limited, and adjust their flood demerits. Print them a warning message if they are over the limit.
...
void ClientCommand(int clientNum) {
	...
	if (command->cmdFlags & CMD_LIVING && (ent->client->ps.stats[STAT_HEALTH] <= 0 || ent->client->sess.spectatorState != SPECTATOR_NOT)) {
...
void Cmd_PrivateMessage_f(gentity_t *ent) {
	...
	if (!count) {
		...
		G_LogPrintf("%s : %d \"%s" S_COLOR_WHITE "\" \"%s\" : ^%c%s\n", (teamonly) ? "TPrivMsg" : "PrivMsg", (int)((ent) ? ent - g_entities : - 1), (ent) ? ent->client->pers.netname : "console", name, color, msg);
	}
}
