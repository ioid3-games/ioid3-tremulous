...
Make a copy of a string for later use. Can safely be called on the same string repeatedly. Redundant on string literals or global
constants.
=======================================================================================================================================
*/
...
void Menu_PostParse(menuDef_t *menu) {
	...
	// push lists to the end of the array as they can potentially be drawn on top of other elements
	for (i = 0; i < menu->itemCount; i++) {
...
/*
=======================================================================================================================================
Item_ListBox_HeightForNumItems
=======================================================================================================================================
*/
...
/*
=======================================================================================================================================
Item_ListBox_NumItemsForItemHeight
=======================================================================================================================================
*/
...
/*
=======================================================================================================================================
Item_ComboBox_MaybeCastToListBox
=======================================================================================================================================
*/
...
/*
=======================================================================================================================================
Item_ComboBox_MaybeUnCastFromListBox
=======================================================================================================================================
*/
...
/*
=======================================================================================================================================
Item_TextField_CalcPaintOffset
=======================================================================================================================================
*/
static void Item_TextField_CalcPaintOffset(itemDef_t *item, char *buff) {
	...
	if (item->cursorPos < editPtr->paintOffset) {
		...
		if (editPtr->maxFieldWidth > 0) {
			// if the cursor is at the end of the string, maximise the amount of the string that's visible
...
const char *Item_Text_Wrap(const char *text, float scale, float width) {
	...
	*paint = '\0';
	...
	while (*p) {
		...
		// note that usage of strcat and strlen is deliberately being avoided here as it becomes surprisingly expensive on larger blocks of text
		...
		*paint = '\0';
		...
		if (paint - out > 0 && * (paint - 1) == '\n') {
			// the line is deliberately broken, clear the color and any current indent
			c[0] = '\0';
			indentWidth = 0.0f;
		} else {
			...
			*paint = '\0';
			...
			// skip leading whitespace on next line and save the last color code
			SkipWhiteSpace(&p, c);
		}
...
void Item_TextField_Paint(itemDef_t *item) {
	...
	if (editing) {
		UI_Text_PaintWithCursor(item->textRect.x + item->textRect.w + offset, item->textRect.y, item->textscale, newColor, buff + editPtr->paintOffset, item->cursorPos - editPtr->paintOffset, cursor, editPtr->maxPaintChars, item->textStyle);
	} else {
...
static bind_t g_bindings[] = {
		...
		{"weapprev", '[', - 1, -1, -1}, {"weapnext", ']', - 1, -1, -1}, {" + button3", K_MOUSE3, - 1, -1, -1}, {" + button4", K_MOUSE4, - 1, -1, -1}, {"vote yes", K_F1, - 1, -1, -1}, {"vote no", K_F2, - 1, -1, -1}, {"teamvote yes", K_F3, - 1, -1, -1}, {"teamvote no", K_F4, - 1, -1, -1}, {"scoresUp", K_KP_PGUP, - 1, -1, -1}, {"scoresDown", K_KP_PGDN, - 1, -1, -1}, {"screenshotJPEG", -1, - 1, -1, -1}, {"messagemode", -1, - 1, -1, -1}, {"messagemode2", -1, - 1, -1, -1}
	};
...
qboolean ItemParse_cvarFloat(itemDef_t *item, int handle) {
	return (PC_String_Parse(handle, &item->cvar) && PC_Float_Parse(handle, &item->typeData.edit->defVal) && PC_Float_Parse(handle, &item->typeData.edit->minVal) && PC_Float_Parse(handle, &item->typeData.edit->maxVal));
}
...
qboolean Item_Parse(int handle, itemDef_t *item) {
	...
	while (1) {
		...
		if (key->param != TYPE_ANY) {
			...
			if (test != key->param) {
				if (test == TYPE_NONE) {
					PC_SourceError(handle, "menu item keyword %s requires type specification", token.string);
				} else {
					PC_SourceError(handle, "menu item keyword %s is incompatible with specified item type", token.string);
				}
