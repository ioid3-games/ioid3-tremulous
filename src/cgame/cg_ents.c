...
//	AxisCopy(cg.autoAxis, ent.axis);
	...
	// the weapons have their origin where they attatch to player models, so we need to offset them or they will rotate eccentricly
...
static void CG_CalcEntityLerpPositions(centity_t *cent) {
	...
	if (cg_projectileNudge.integer && !cg.demoPlayback && cent->currentState.eType == ET_MISSILE && !(cg.snap->ps.pm_flags & PMF_FOLLOW)) {
		timeshift = cg.ping;
	}
...
static void CG_CEntityPVSEnter(centity_t *cent) {
	...
	// make sure that the buildable animations are in a consistent state when a buildable enters the PVS
