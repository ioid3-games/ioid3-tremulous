...
static void CG_DrawBlips(rectDef_t *rect, vec3_t origin, vec4_t colour) {
	...
	if (drawOrigin[2] > 0) {
		CG_DrawPic(rect->x + (rect->w / 2) - (STALKWIDTH / 2) - drawOrigin[0], rect->y + (rect->h / 2) + drawOrigin[1] - drawOrigin[2], STALKWIDTH, drawOrigin[2], cgs.media.scannerLineShader);
	} else {
		CG_DrawPic(rect->x + (rect->w / 2) - (STALKWIDTH / 2) - drawOrigin[0], rect->y + (rect->h / 2) + drawOrigin[1], STALKWIDTH, -drawOrigin[2], cgs.media.scannerLineShader);
	}

	CG_DrawPic(rect->x + (rect->w / 2) - (BLIPX / 2) - drawOrigin[0], rect->y + (rect->h / 2) - (BLIPY / 2) + drawOrigin[1] - drawOrigin[2], BLIPX, BLIPY, cgs.media.scannerBlipShader);
...
static void CG_DrawDir(rectDef_t *rect, vec3_t origin, vec4_t colour) {
	...
	CG_DrawPic(rect->x + (rect->w / 2) - (BLIPX2 / 2) - drawOrigin[0] * (rect->w / 2), rect->y + (rect->h / 2) - (BLIPY2 / 2) + drawOrigin[1] * (rect->h / 2), BLIPX2, BLIPY2, cgs.media.scannerBlipShader);
	trap_R_SetColor(NULL);
}
...
void THZ_DrawScanner(rectDef_t *rect) {
	...
	for (i = 0; i < entityPositions.numHumanBuildables; i++) {
		...
		if (VectorLength(relOrigin) < thz_radarrange.integer) {
			...
			CG_FillRect(rect->x + (rect->w / 2) + -drawOrigin[0] - 5, rect->y + (rect->h / 2) + drawOrigin[1] - 5, 10, 10, buildcolor);
		}
	}
	// humans
	for (i = 0; i < entityPositions.numHumanClients; i++) {
		...
		if (VectorLength(relOrigin) < thz_radarrange.integer) {
			...
			CG_FillRect(rect->x + (rect->w / 2) + -drawOrigin[0] - 3, rect->y + (rect->h / 2) + drawOrigin[1] - 3, 6, 6, humancolor);
		}
	}
	// alien structures
	for (i = 0; i < entityPositions.numAlienBuildables; i++) {
		...
		if (VectorLength(relOrigin) < thz_radarrange.integer) {
			...
			CG_FillRect(rect->x + (rect->w / 2) + -drawOrigin[0] - 5, rect->y + (rect->h / 2) + drawOrigin[1] - 5, 10, 10, buildcolor2);
		}
	}
	// aliens
	for (i = 0; i < entityPositions.numAlienClients; i++) {
		...
		if (VectorLength(relOrigin) < thz_radarrange.integer) {
			...
			CG_FillRect(rect->x + (rect->w / 2) + -drawOrigin[0] - 3, rect->y + (rect->h / 2) + drawOrigin[1] - 3, 6, 6, aliencolor);
		}
	}
}
