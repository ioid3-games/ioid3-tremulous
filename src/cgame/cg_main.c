...
static const char *CG_FeederItemText(int feederID, int index, int column, qhandle_t *handle) {
	...
	*handle = -1;
	...
	if (cg.intermissionStarted && CG_ClientIsReady(sp->client)) {
		showIcons = qfalse;
	} else if (cg.snap->ps.pm_type == PM_SPECTATOR || cg.snap->ps.pm_type == PM_NOCLIP || cg.snap->ps.pm_flags & PMF_FOLLOW || team == cg.snap->ps.stats[STAT_TEAM] || cg.intermissionStarted) {
		showIcons = qtrue;
	}
...
void CG_AssetCache(void) {
	...
	for (i = 0; i < cgDC.Assets.emoticonCount; i++) {
		cgDC.Assets.emoticons[i].shader = trap_R_RegisterShaderNoMip(va("emoticons/%s_%dx1.tga", cgDC.Assets.emoticons[i].name, cgDC.Assets.emoticons[i].width));
	}
}
