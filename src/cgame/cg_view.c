...
void CG_OffsetThirdPersonView(void) {
	...
	if (cg.demoPlayback || ((cg.snap->ps.pm_flags & PMF_FOLLOW) && (cg.predictedPlayerState.stats[STAT_HEALTH] > 0))) {
		...
		if (!(cg.snap->ps.stats[STAT_STATE] & SS_WALLCLIMBING) || !BG_RotateAxis(cg.snap->ps.grapplePoint, axis, rotaxis, qfalse, cg.snap->ps.eFlags & EF_WALLCLIMBCEILING)) {
...
#define STRUGGLE_TIME 250
	...
	if ((cg.predictedPlayerEntity.currentState.eFlags & EF_POISONCLOUDED) && (cg.time - cg.poisonedTime < PCLOUD_DISORIENT_DURATION) && !(cg.snap->ps.pm_flags & PMF_FOLLOW)) {
...
static int CG_CalcFov(void) {
	...
	if (cg.predictedPlayerState.pm_type == PM_INTERMISSION || (cg.snap->ps.persistant[PERS_SPECSTATE] != SPECTATOR_NOT) || (cg.renderingThirdPerson)) {
		...
		if (cg.spawnTime > (cg.time - FOVWARPTIME) && BG_ClassHasAbility(cg.predictedPlayerState.stats[STAT_CLASS], SCA_FOVWARPS)) {
	...
	if ((cg.predictedPlayerEntity.currentState.eFlags & EF_POISONCLOUDED) && (cg.time - cg.poisonedTime < PCLOUD_DISORIENT_DURATION) && cg.predictedPlayerState.stats[STAT_HEALTH] > 0 && !(cg.snap->ps.pm_flags & PMF_FOLLOW)) {
