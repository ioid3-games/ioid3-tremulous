...
void CG_Menu(int menu, int arg) {
	...
	switch (menu) {
		...
		case MN_H_UNKNOWNITEM:
			...
			break;
		case MN_A_TEAMFULL:
			longMsg = "The alien team has too many players. Please wait until slots become available or join the human team.";
			shortMsg = "The alien team has too many players";
			type = DT_COMMAND;
			break;
		case MN_H_TEAMFULL:
			longMsg = "The human team has too many players. Please wait until slot become available or join the alien team.";
			shortMsg = "The human team has too many players";
			type = DT_COMMAND;
			break;
		case MN_A_TEAMLOCKED:
			longMsg = "The alien team is locked. You cannot join the aliens at this time.";
			shortMsg = "The alien team is locked";
			type = DT_COMMAND;
			break;
		case MN_H_TEAMLOCKED:
			longMsg = "The human team is locked. You cannot join the humans at this time.";
			shortMsg = "The human team is locked";
			type = DT_COMMAND;
			break;
		case MN_PLAYERLIMIT:
			longMsg = "The maximum number of playing clients has been reached. Please wait until slots become available.";
		...
		case MN_CMD_CHEAT_TEAM:
			shortMsg = "Cheats are not enabled on this server, so you may not use this command while on a team";
			type = DT_COMMAND;
			break;
		case MN_CMD_TEAM:
			// longMsg = "You must be on a team to perform this action. Join the alien or human team and try again.";
			shortMsg = "Join a team first";
			type = DT_COMMAND;
			break;
		case MN_CMD_SPEC:
			// longMsg = "You may not perform this action while on a team. Become a spectator before trying again.";
		...
		case MN_CMD_LIVING:
			...
			break;
		case MN_B_NOROOM:
			longMsg = "There is no room to build here. Move until the structure turns translucent green, indicating a valid build location.";
			shortMsg = "There is no room to build here";
			type = DT_BUILD;
			break;
		case MN_B_NORMAL:
			longMsg = "Cannot build on this surface. The surface is too steep or unsuitable for building. Please choose another site for this structure.";
		...
		case MN_B_LASTSPAWN:
			longMsg = "This action would remove your team's last spawn point, which often quickly results in a loss. Try building more spawns.";
			shortMsg = "You may not deconstruct the last spawn";
			break;
		case MN_B_SUDDENDEATH:
			longMsg = "Neither team has prevailed after a certain time and the game has entered Sudden Death. During Sudden Death building is not allowed.";
			shortMsg = "Cannot build during Sudden Death";
			type = DT_BUILD;
			break;
		case MN_B_REVOKED:
			longMsg = "Your teammates have lost faith in your ability to build for the team. You will not be allowed to build until your team votes to reinstate your building rights.";
			shortMsg = "Your building rights have been revoked";
			type = DT_BUILD;
			break;
		case MN_B_SURRENDER:
			longMsg = "Your team has decided to admit defeat and concede the game: traitors and cowards are not allowed to build."; // too harsh?
			shortMsg = "Building is denied to traitorous cowards";
			break;
		case MN_H_NOBP:
			if (cgs.markDeconstruct) {
				longMsg = "There is no power remaining. Free up power by existing buildable objects.";
			} else {
				longMsg = "There is no power remaining. Free up power by deconstructing existing buildable objects.";
			}
		...
		case MN_H_NOTPOWERED:
			longMsg = "This buildable is not powered. Build a Reactor and/or Repeater in order to power it.";
			shortMsg = "This buildable is not powered";
			type = DT_BUILD;
			break;
		case MN_H_ONEREACTOR:
			longMsg = "There can only be one Reactor. Deconstruct the existing one if you wish to move it.";
			shortMsg = "There can only be one Reactor";
			type = DT_BUILD;
			break;
		case MN_H_NOPOWERHERE:
			longMsg = "There is no power here. If available, a Repeater may be used to transmit power to this location.";
			shortMsg = "There is no power here";
			type = DT_BUILD;
			break;
		case MN_H_NODCC:
			longMsg = "There is no Defense Computer. A Defense Computer is needed to build this.";
		...
		case MN_H_RPTPOWERHERE:
			...
			break;
		case MN_H_NOSLOTS:
			longMsg = "You have no room to carry this. Please sell any conflicting upgrades before purchasing this item.";
			shortMsg = "You have no room to carry this";
			type = DT_ARMOURYEVOLVE;
			break;
		case MN_H_NOFUNDS:
			longMsg = "Insufficient funds. You do not have enough credits to perform this action.";
			shortMsg = "Insufficient funds";
			type = DT_ARMOURYEVOLVE;
			break;
		case MN_H_ITEMHELD:
			longMsg = "You already hold this item. It is not possible to carry multiple items of the same type.";
			shortMsg = "You already hold this item";
			type = DT_ARMOURYEVOLVE;
			break;
		case MN_H_NOARMOURYHERE:
			longMsg = "You must be near a powered Armoury in order to purchase weapons, upgrades or ammunition.";
			shortMsg = "You must be near a powered Armoury";
			type = DT_ARMOURYEVOLVE;
			break;
		case MN_H_NOENERGYAMMOHERE:
			longMsg = "You must be near a Reactor or a powered Armoury or Repeater in order to purchase energy ammunition.";
			shortMsg = "You must be near a Reactor or a powered Armoury or Repeater";
			type = DT_ARMOURYEVOLVE;
			break;
		case MN_H_NOROOMBSUITON:
			longMsg = "There is not enough room here to put on a Battle Suit. Make sure you have enough head room to climb in.";
			shortMsg = "Not enough room here to put on a Battle Suit";
			type = DT_ARMOURYEVOLVE;
			break;
		case MN_H_NOROOMBSUITOFF:
			longMsg = "There is not enough room here to take off your Battle Suit. Make sure you have enough head room to climb out.";
			shortMsg = "Not enough room here to take off your Battle Suit";
			type = DT_ARMOURYEVOLVE;
			break;
		case MN_H_ARMOURYBUILDTIMER:
			longMsg = "You are not allowed to buy or sell weapons until your build timer has expired.";
			shortMsg = "You can not buy or sell weapons until your build timer expires";
			type = DT_ARMOURYEVOLVE;
			break;
		...
		case MN_H_UNKNOWNSPAWNITEM:
			...
			break;
		case MN_A_NOCREEP:
			longMsg = "There is no creep here. You must build near existing Eggs or the Overmind. Alien structures will not support themselves.";
			shortMsg = "There is no creep here";
			type = DT_BUILD;
			break;
		case MN_A_NOOVMND:
			longMsg = "There is no Overmind. An Overmind must be built to control the structure you tried to place.";
			shortMsg = "There is no Overmind";
			type = DT_BUILD;
			break;
		case MN_A_ONEOVERMIND:
			longMsg = "There can only be one Overmind. Deconstruct the existing one if you wish to move it.";
			shortMsg = "There can only be one Overmind";
			type = DT_BUILD;
			break;
		case MN_A_NOBP:
			longMsg = "The Overmind cannot control any more structures. Deconstruct existing structures to build more.";
		...
		case MN_A_NOEROOM:
			...
			break;
		case MN_A_TOOCLOSE:
			longMsg = "This location is too close to the enemy to evolve. Move away from the enemy's presence and try again.";
			shortMsg = "This location is too close to the enemy to evolve";
			type = DT_ARMOURYEVOLVE;
			break;
		case MN_A_NOOVMND_EVOLVE:
			longMsg = "There is no Overmind. An Overmind must be built to allow you to upgrade.";
			shortMsg = "There is no Overmind";
			type = DT_ARMOURYEVOLVE;
			break;
...
static void CG_Say(int clientNum, saymode_t mode, const char *text) {
	...
	switch (mode) {
		...
		case SAY_ADMINS_PUBLIC:
			CG_Printf("%s%s%s%s" S_COLOR_WHITE "%s %c" S_COLOR_MAGENTA "%s\n", ignore, prefix, (mode == SAY_ADMINS) ? "[ADMIN]" : "[PLAYER]", name, maybeColon, INDENT_MARKER, text);
			break;
		case SAY_AREA:
		...
		case SAY_TPRIVMSG:
			...
			CG_Printf("%s%s[%s" S_COLOR_WHITE "->%s" S_COLOR_WHITE "]%s %c%s%s\n", ignore, prefix, name, cgs.clientinfo[cg.clientNum].name, maybeColon, INDENT_MARKER, color, text);

			if (!ignore[0]) {
				CG_CenterPrint(va("%sPrivate message from: " S_COLOR_WHITE "%s", color, name), 200, GIANTCHAR_WIDTH * 4);
...
Return the voice indexed voice track or print errors quietly to console in case someone is on an unpure server and wants to know which
voice pak is missing or incomplete.
=======================================================================================================================================
*/
static voiceTrack_t *CG_VoiceTrack(char *voice, int cmd, int track) {
	...
	if (!c) {
		CG_Printf("[skipnotify]WARNING: could not find command %d in voice \"%s\"\n", cmd, voice);
		return NULL;
	}
	...
	if (!t) {
		CG_Printf("[skipnotify]WARNING: could not find track %d for command %d in voice \"%s\"\n", track, cmd, voice);
		return NULL;
	}
...
static void CG_ServerCommand(void) {
	...
	cmd = CG_Argv(0);
	command = bsearch(cmd, svcommands, ARRAY_LEN(svcommands), sizeof(svcommands[0]), cmdcmp);
