	...
	while (1) {
		...
		if (!Q_stricmp(token, "{")) {
			...
			if (!wi->weaponModel3rdPerson) {
				CG_Printf(S_COLOR_RED "ERROR: 3rd person weapon model not found %s\n", token);
			}
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent) {
	...
	if (!weapon->registered) {
		Com_Printf(S_COLOR_YELLOW "WARNING: CG_AddPlayerWeapon : weapon %d(%s) is not registered\n", weaponNum, BG_Weapon(weaponNum)->name);
		return;
	}
	// add the weapon
...
void CG_AddViewWeapon(playerState_t *ps) {
	...
	if (!wi->registered) {
		Com_Printf(S_COLOR_YELLOW "WARNING: CG_AddViewWeapon : weapon %d(%s) is not registered\n", weapon, BG_Weapon(weapon)->name);
		return;
	}
...
void CG_DrawItemSelect(rectDef_t *rect, vec4_t color) {
	...
	for (i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++) {
		...
		if (!cg_weapons[i].registered) {
			Com_Printf(S_COLOR_YELLOW "WARNING: CG_DrawItemSelect : weapon %d(%s) is not registered\n", i, BG_Weapon(i)->name);
			continue;
		}
	...
	for (i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++) {
		...
		if (!cg_upgrades[i].registered) {
			Com_Printf(S_COLOR_YELLOW "WARNING: CG_DrawItemSelect : upgrade %d(%s) is not registered\n", i, BG_Upgrade(i)->name);
			continue;
		}
...
=======================================================================================================================================
*/
void CG_MissileHitWall(weapon_t weaponNum, weaponMode_t weaponMode, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType, int charge) {
	qhandle_t mark = 0;
	qhandle_t ps = 0;
	int c;
	float radius = 1.0f;
...
=======================================================================================================================================
*/
void CG_MissileHitEntity(weapon_t weaponNum, weaponMode_t weaponMode, vec3_t origin, vec3_t dir, int entityNum, int charge) {
	vec3_t normal;
	weaponInfo_t *weapon = &cg_weapons[weaponNum];
	...
	if (weapon->wim[weaponMode].alwaysImpact) {
		...
		} else if (cg_entities[entityNum].currentState.eType == ET_BUILDABLE && BG_Buildable(cg_entities[entityNum].currentState.modelindex)->team == TEAM_ALIENS) {
			// alien buildables
			sound = IMPACTSOUND_FLESH;
		} else {
...
static void CG_ShotgunPattern(vec3_t origin, vec3_t origin2, int seed, int otherEntNum) {
	...
	for (i = 0; i < SHOTGUN_PELLETS; i++) {
		...
		if (!(tr.surfaceFlags & SURF_NOIMPACT)) {
			if (cg_entities[tr.entityNum].currentState.eType == ET_PLAYER || cg_entities[tr.entityNum].currentState.eType == ET_BUILDABLE) {
