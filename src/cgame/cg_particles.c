...
static void CG_DestroyParticle(particle_t *p, vec3_t impactNormal) {
	...
	p->valid = qfalse;
	// this gives other systems a couple of frames to realise the particle is gone
	...
	for (i = 0; i < MAX_PARTICLES; i++) {
		...
		if (!p->valid && cg.clientFrame > p->frameWhenInvalidated + 1) {
			...
			if (CG_AttachmentVelocity(&ps->attachment, attachmentVelocity)) {
				VectorMA(p->velocity, CG_RandomiseValue(bp->velMoveValues.parentVelFrac, bp->velMoveValues.parentVelFracRandFrac), attachmentVelocity, p->velocity);
			}
...
Check if there are any ejectors that should be introducing new particles.
=======================================================================================================================================
*/
		...
		if (pe->valid) {
			...
			if (!ps->lazyRemove) {
				while (pe->nextEjectionTime <= cg.time && (pe->count > 0 || pe->totalParticles == PARTICLES_INFINITE)) {
					...
					pe->nextEjectionTime = cg.time + (int)CG_RandomiseValue(CG_LerpValues(pe->ejectPeriod.initial, pe->ejectPeriod.final, lerpFrac), pe->ejectPeriod.randFrac);
				}
			}
...
static particleEjector_t *CG_SpawnNewParticleEjector(baseParticleEjector_t *bpe, particleSystem_t *parent) {
	...
	for (i = 0; i < MAX_PARTICLE_EJECTORS; i++) {
		...
		if (!pe->valid) {
			...
			pe->count = pe->totalParticles = (int)rint(CG_RandomiseValue((float)bpe->totalParticles, bpe->totalParticlesRandFrac));
			pe->valid = qtrue;
		...
		if (!Q_stricmpn(bps->name, name, MAX_QPATH)) {
			...
			for (j = 0; j < bps->numEjectors; j++) {
				...
				for (l = 0; l < bpe->numParticles; l++) {
					...
					if (bp->childSystemName[0] != '\0') {
						// don't care about a handle for children since the system deals with it
...
static qboolean CG_ParseParticle(baseParticle_t *bp, char **text_p) {
	...
	while (1) {
		...
		if (!Q_stricmp(token, "bounce")) {
			...
			CG_ParseValueAndVariance(token, &bp->velMoveValues.mag, &bp->velMoveValues.magRandFrac, qfalse);
			...
			CG_ParseValueAndVariance(token, &bp->accMoveValues.mag, &bp->accMoveValues.magRandFrac, qfalse);
			...
			for (i = 0; i <= 2; i++) {
				...
				CG_ParseValueAndVariance(token, &bp->displacement[i], &bp->randDisplacement[i], qtrue);
			}
...
This doesn't actually invalidate anything, it just stops particle ejectors from producing new particles so the garbage collector will
eventually remove this system.
However is does set the pointer to NULL so the user is unable to manipulate this particle system any longer.
=======================================================================================================================================
*/
...
static void CG_GarbageCollectParticleSystems(void) {
	...
	for (i = 0; i < MAX_PARTICLE_SYSTEMS; i++) {
		...
		// check systems where the parent cent has left the PVS (local player entity is always valid)
...
#define MAX_ACC_RADIUS 1000.0f
	...
	if (bp->bounceMarkName[0] && p->bounceMarkCount > 0) {
		CG_ImpactMark(bp->bounceMark, trace.endpos, trace.plane.normal, random() * 360, 1, 1, 1, 1, qtrue, bp->bounceMarkRadius, qfalse);
		p->bounceMarkCount--;
	}
...
static void CG_RenderParticle(particle_t *p) {
	...
	if (bp->numFrames) { // shader based
		...
		re.shaderRGBA[3] = (byte)((float)0xFF * CG_LerpValues(p->alpha.initial, p->alpha.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->alpha.delay)));
		re.radius = scale;
		re.rotation = CG_LerpValues(p->rotation.initial, p->rotation.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->rotation.delay));
	...
	if (bp->dynamicLight && !(re.renderfx & RF_THIRD_PERSON)) {
		trap_R_AddLightToScene(p->origin, CG_LerpValues(p->dLightRadius.initial, p->dLightRadius.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->dLightRadius.delay)), (float)bp->dLightColor[0] / (float)0xFF, (float)bp->dLightColor[1] / (float)0xFF, (float)bp->dLightColor[2] / (float)0xFF);
	}
