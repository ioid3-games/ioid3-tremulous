...
static void CG_RenderBeam(trailBeam_t *tb) {
	...
	if (bts->thirdPersonOnly && (CG_AttachmentCentNum(&ts->frontAttachment) == cg.snap->ps.clientNum || CG_AttachmentCentNum(&ts->backAttachment) == cg.snap->ps.clientNum) && !cg.renderingThirdPerson) {
		return;
	}
...
Removes a node from a beam. Returns the new head.
=======================================================================================================================================
*/
...
Prepend a new beam node to the front of a beam. Returns the new node.
=======================================================================================================================================
*/
...
Append a new beam node to the back of a beam. Returns the new node.
=======================================================================================================================================
*/
...
static void CG_GarbageCollectTrailSystems(void) {
	...
	for (i = 0; i < MAX_TRAIL_SYSTEMS; i++) {
		...
		// lifetime expired
		if (ts->destroyTime <= 0 && ts->class->lifeTime && ts->birthTime + ts->class->lifeTime < cg.time) {
			trailSystem_t *tempTS = ts;

			CG_DestroyTrailSystem(&tempTS);

			if (cg_debugTrails.integer >= 1) {
				CG_Printf("TS %s expired(born %d, lives %d, now %d)\n", ts->class->name, ts->birthTime, ts->class->lifeTime, cg.time);
			}
		}
