...
namespace script {
		...
		public:
				virtual const char *what() const throw() {return "cvar name is invalid"; }
		};

		class CvarNotFound : public std::exception {
		public:
				virtual const char *what() const throw() {return "cvar does not exist"; }
		};

		class CvarReadOnly : public std::exception {
		public:
				virtual const char *what() const throw() {return "cvar is readonly"; }
		};

		class CvarWriteProtected : public std::exception {
		public:
				virtual const char *what() const throw() {return "cvar is write protected"; }
		};

		class CvarCheatProtected : public std::exception {
		public:
				virtual const char *what() const throw() {return "cvar is cheat protected"; }
		};

		class CvarLatchedUnsupported : public std::exception {
		public:
				virtual const char *what() const throw() {return "modifying latched cvars is unsupported"; }
		};
		...
		public:
				...
				Cvar(std::string name) {my = Cvar_Get(name.c_str(), "", 0); }

				Cvar(std::string name, std::string value) {my = Cvar_Get(name.c_str(), value.c_str(), CVAR_ARCHIVE); }

				Cvar(std::string name, std::string value, int flags) {my = Cvar_Get(name.c_str(), value.c_str(), flags); }
				...
				std::string get_value() {return my->string; }

				std::string get_key() {return my->name; }
		};

		namespace cvar {
				static inline void init(sol::state * lua) {
						lua->new_usertype < Cvar > (
										"cvar", sol::constructors < sol::types < std::string >, sol::types < std::string, std::string >, sol::types < std::string, std::string, int >> (), "value", sol::property(&Cvar::get_value, &Cvar::set_value), "key", sol::property(&Cvar::get_key)
					);
				}
		};
};
